...
#define openDoorPin 51
#define successPin 49
...
void setup() {
  ...
  pinMode(s1lr_g, INPUT);
  pinMode(s1lr_r, INPUT);
  pinMode(s2lr_g, INPUT);
  pinMode(s2lr_r, INPUT);
  pinMode(s1ud_g, INPUT);
  pinMode(s1ud_r, INPUT);
  pinMode(s2ud_g, INPUT);
  pinMode(s2ud_r, INPUT);
...
void loop() {
  ...
  if (token == 0) {
    ...
    if (pixy.ccc.numBlocks > 1) {
      ...
      yCoordinateTracking = pixy.ccc.blocks[0].m_y + 46;
...
//      }
  ...
  if (digitalRead(s1lr_g) != HIGH) {
    Serial.println("s1lr_g");
  }
  if (digitalRead(s1lr_r) != HIGH) {
    Serial.println("s1lr_r");
  }
  if (digitalRead(s2lr_g) != HIGH) {
    Serial.println("s2lr_g");
  }
  if (digitalRead(s2lr_r) != HIGH) {
    Serial.println("s2lr_r");
  }
  if (digitalRead(s1ud_g) != HIGH) {
    Serial.println("s1ud_g");
  }
  if (digitalRead(s1ud_r) != HIGH) {
    Serial.println("s1ud_r");
  }
  if (digitalRead(s2ud_g) != HIGH) {
    Serial.println("s2ud_g");
  }
  if (digitalRead(s2ud_r) != HIGH) {
    Serial.println("s2ud_r");
  }

  if (digitalRead(successPin) == HIGH) {
  ...
  if (reverseTime == 1) {
    ...
    while ((digitalRead(s1lr_r) == HIGH) || (digitalRead(s2lr_r) == HIGH)) {
...
//        stepper4.setupRelativeMoveInSteps(stepSize);
//      }
      while ((!stepper2.motionComplete()) || (!stepper3.motionComplete())) {
//        stepper1.processMovement();
        stepper2.processMovement();
        stepper3.processMovement();
//        stepper4.processMovement();
