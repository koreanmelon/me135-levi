...
void loop() {
  ...
  if (token == 0) {
    ...
    if (pixy.ccc.numBlocks) {
      xCoordinate = pixy.ccc.blocks[0].m_x;
      yCoordinate = pixy.ccc.blocks[0].m_y;
      token = 1;
    }
  }
...
//      stepper4.moveRelativeInSteps(-stepSize);
  ...
  if (BS1 == HIGH) {
     stepper1.setupRelativeMoveInSteps(ButtonStepSize);// Set the speed in steps per second:
  } else if (BS2 == HIGH){
     stepper1.setupRelativeMoveInSteps(-ButtonStepSize);// Set the speed in steps per second:
  }
  if (BS3 == HIGH) {
   ...
   if (BS5 == HIGH) {
     stepper3.setupRelativeMoveInSteps(2*ButtonStepSize);// Set the speed in steps per second:
  } else if (BS6 == HIGH){
     stepper3.setupRelativeMoveInSteps(-2*ButtonStepSize);// Set the speed in steps per second:
  }
   if (BS7 == HIGH) {
